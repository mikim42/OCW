...
/*   Updated: 2019/05/10 14:17:55 by mikim            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
//	*p = 10;
    ...
    //Basic movements - move only if left and right of destination is movable
    if(movable(MAP[y + 1][x], my_num) && (movable(MAP[y + 1][x - 1], my_num) || movable(MAP[y + 1][x + 1], my_num)))
        return UP;
    if(movable(MAP[y][x + 1], my_num) && (movable(MAP[y + 1][x + 1], my_num) || movable(MAP[y - 1][x + 1], my_num)))
        return RIGHT;
    if(movable(MAP[y - 1][x], my_num) && (movable(MAP[y - 1][x - 1], my_num) || movable(MAP[y - 1][x + 1], my_num)))
        return DOWN;
    if(movable(MAP[y][x - 1], my_num) && (movable(MAP[y + 1][x - 1], my_num) || movable(MAP[y - 1][x - 1], my_num)))
